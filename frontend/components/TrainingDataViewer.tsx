...
export default function TrainingDataViewer({ botId }: TrainingDataViewerProps) {
  ...
  if (error) {
    return (
      <div className="bg-[#F43F5E]/10 border border-red-200 rounded-lg p-4">
        <p className="text-red-800">Error loading training data: {error}</p>
        <button
          onClick={loadTrainingData}
          className="mt-2 text-sm text-[#F43F5E] hover:text-red-800 underline"
        >
          Try again
        </button>
      </div>
  ...
  if (!trainingData || trainingData.totalItems === 0) {
    return (
      <div className="bg-[#F59E0B]/10 border border-yellow-200 rounded-lg p-4">
  ...
  return (
    <div className="bg-[#1E293B] rounded-lg shadow-lg p-6">
      <div className="flex justify-between items-center mb-6">
        <h3 className="text-xl font-bold text-[#F8FAFC]">Training Data</h3>
        <button
          onClick={loadTrainingData}
      ...
      {trainingData.textTraining.length > 0 && (
        <div className="mb-6">
          <h4 className="text-lg font-semibold text-[#94A3B8] mb-3">
                ...
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-[#94A3B8] mb-2">
                      <div className="text-xs font-semibold text-[#94A3B8] mb-1">📄 Extracted Text:</div>
                      {expandedItems.has(item.id) ? (
                        <div className="whitespace-pre-wrap bg-[#0F172A] p-3 rounded border max-h-96 overflow-y-auto font-mono text-xs">
                          {item.fullContent || item.content || 'No content extracted'}
                        </div>
                      ) : (
                        <div className="bg-[#0F172A] p-3 rounded border">
