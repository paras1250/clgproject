...
const ChatbotMockup = () => (
    <div className="relative w-full max-w-[420px]">
        ...
        <div className="chatbot-container h-[550px] bg-white rounded-3xl shadow-2xl overflow-hidden border border-gray-100 flex flex-col">
            <div className="chatbot-container-header bg-white p-4 flex items-center gap-3 border-b border-gray-100">
                <img src="/assets/ai_agent_avatar.png" className="w-8 h-8 rounded-full bg-gray-50 p-0.5 object-cover" alt="AI Agent" />
                <span className="noupe-medium text-xs font-semibold text-gray-800">AI Assistant</span>
                <svg className="ml-auto w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"></path></svg>
            </div>

            <div className="chatbot-container-body flex-1 p-4 bg-gray-50 flex flex-col gap-4">
                <div className="noupe-message user self-end h-8 w-2/3 bg-gray-200 rounded-lg animate-pulse"></div>

                <div className="noupe-message agent flex flex-col gap-1 items-start">
                    <div className="bg-white p-3 rounded-2xl rounded-tl-none shadow-sm max-w-[90%]">
                        <div className="thinking-dots flex gap-1 mb-1">
                            <div className="w-1 h-1 bg-gray-300 rounded-full animate-bounce"></div>
                            <div className="w-1 h-1 bg-gray-300 rounded-full animate-bounce [animation-delay:0.1s]"></div>
                    ...
                    <span className="text-[10px] noupe-bold text-blue-600 uppercase ml-1">Noupe AI Answered</span>
                </div>

                <div className="noupe-message user self-end h-8 w-1/2 bg-gray-200 rounded-lg animate-pulse"></div>
...
const FeatureCard = ({ title, description, image, fullWidth, bgClass = "bg-[#FDF6F0]" }: { title: string; description: string; image: string; fullWidth?: boolean; bgClass?: string }) => (
    <div className={`${bgClass} rounded-[2.5rem] p-8 md:p-12 flex flex-col items-center gap-10 text-center ${fullWidth ? 'md:col-span-2' : ''}`}>
        <div className="max-w-2xl px-4">
            <h3 className="text-3xl md:text-4xl noupe-bold mb-4 text-[#0A0807] font-bold">{title}</h3>
            <p className="text-[#595755] text-lg md:text-xl noupe-medium leading-relaxed">{description}</p>
        </div>
        <div className="w-full">
            <img src={`/assets/${image}`} alt={title} className="w-full h-auto rounded-3xl shadow-lg" />
        </div>
    </div>
);
    ...
    <section className="relative px-6 md:px-20 py-10 md:py-24 max-w-[1400px] mx-auto min-h-[85vh] flex items-center">
        ...
        <div className="grid lg:grid-cols-[1.2fr_1fr] gap-16 items-center w-full">
            <div className="text-left animate-in fade-in slide-in-from-left-8 duration-700">
                <h1 className="text-5xl md:text-[5.5rem] noupe-semibold leading-[1.05] tracking-tight mb-8 text-[#0A0807] font-bold">
                    AI Chatbot for your site
                </h1>
                <p className="text-xl md:text-2xl text-[#595755] noupe-medium leading-relaxed mb-12 max-w-xl">
                ...
                <div className="flex flex-col gap-4 max-w-md">
                    <Link href="/login" className="noupe-button noupe-button-google py-4 px-6 text-xl relative group bg-white border border-gray-200 rounded-full shadow-sm hover:shadow-md transition-all flex items-center justify-center">
                    ...
                    <Link href="/login" className="noupe-button noupe-button-primary py-4 px-6 text-xl group bg-[#0A0807] text-white rounded-full hover:bg-opacity-90 transition-all flex items-center justify-center">
...
const Footer = () => (
    <footer className="bg-[#0A0807] text-white py-20 px-6">
        <div className="max-w-[1200px] mx-auto flex flex-col items-center gap-12 text-center text-gray-400">
            <div className="flex flex-wrap justify-center gap-x-10 gap-y-4">
                {["Terms & Conditions", "Privacy Policy", "About Us", "Contact Us", "Pricing", "Embed Guide"].map(item => (
                    <Link key={item} href="#" className="hover:text-[#D200D3] transition-colors font-medium">{item}</Link>
                ))}
            </div>
            <div className="max-w-3xl text-sm leading-relaxed text-gray-500">
                Noupe is a <Link href="#" className="text-white hover:underline">free AI chatbot builder</Link> that creates AI chatbots in minutes, trusted by businesses worldwide. Noupe (Jotform Inc.) 4 Embarcadero Center, Suite 780, San Francisco CA 94111.
            </div>
        </div>
    </footer>
);
...
export default function LandingPage() {
    return (
        <div className="min-h-screen selection:bg-[#D200D3]/20 font-sans">
            <Head>
                <title>Noupe - AI Chatbot Builder</title>
                <meta name="description" content="Build your own AI chatbot in minutes." />
            </Head>

            <header className="px-6 md:px-12 py-8 flex justify-between items-center relative md:sticky top-0 z-50 noupe-blur-white bg-white/80 backdrop-blur-md">
                <Logo />
                <div className="flex items-center gap-6">
                    <nav className="hidden lg:flex items-center gap-10 mr-4">
                        {["Solutions", "Blog", "Pricing"].map(item => (
                            <Link key={item} href={item === "Pricing" ? "/pricing" : "#"} className="noupe-medium text-[#0A0807] hover:text-[#D200D3] transition-colors text-lg font-medium">{item}</Link>
                        ))}
                        <Link href="/login" className="noupe-medium text-[#0A0807] hover:text-[#D200D3] transition-colors text-lg font-medium">Login</Link>
                    </nav>
                    <Link href="/login" className="noupe-button noupe-button-primary px-6 py-3 text-md hidden md:flex bg-[#0A0807] text-white rounded-full hover:shadow-lg transition-all font-semibold">Sign Up for Free</Link>
                    <div className="w-10 h-10 bg-[#0A0807] rounded-full md:flex hidden flex-col items-center justify-center gap-0.5 cursor-pointer hover:bg-opacity-80 transition-all">
            ...
            <section className="bg-[#0A0807] text-white py-32 px-6">
                <div className="max-w-[1100px] mx-auto text-center mb-24">
                    <h2 className="text-5xl md:text-[5rem] noupe-semibold leading-[1.05] mb-8 font-bold">
                        Key features that <span className="italic font-normal text-[#D200D3]">power</span> your business
                    </h2>
            ...
            <section className="py-32 px-6 max-w-[1200px] mx-auto text-center">
                <h2 className="text-4xl md:text-6xl noupe-semibold mb-16 tracking-tight font-bold">
                    Why you will <span className="italic font-normal text-[#D200D3]">love Noupe</span>
                </h2>
                <div className="grid md:grid-cols-2 gap-6">
                    {[
                        ...
                        <div key={idx} className="bg-white p-8 rounded-[2rem] flex items-center gap-6 shadow-sm hover:shadow-md transition-shadow text-left border border-gray-100">
                            <div className="w-16 h-16 bg-[#F3E6F5] rounded-2xl flex items-center justify-center shrink-0 text-[#D200D3]">
                            ...
                            <span className="text-xl noupe-bold text-[#0A0807] font-bold">{item.text}</span>
                        </div>
                    ))}
                </div>
            </section>
